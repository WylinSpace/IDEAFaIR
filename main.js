...
import { initUI, isPointerFeatureActive, updateStatus } from './modules/ui.js';
...
let handCooldown = 0; // Prevent rapid triggering
...
async function initApp() {
  try {
    ...
    // Load Vision (MediaPipe)
    await initVision();
    updateStatus("Vision Loaded...");
    
    // Load Object Detection (COCO-SSD from global space loaded via HTML)
    try {
    ...
    // Tesseract Worker Setup (Global from loaded HTML CDN)
    try {
...
window.addEventListener('error', function(e) {
    ...
    if (lbl) {
        lbl.innerText = "JS ERROR: " + e.message;
        lbl.style.color = "red";
...
bleBtn.addEventListener('click', async () => {
  if (bleBtn.innerText === "Disconnect") {
    ...
    if (success) {
      bleBtn.innerText = "Disconnect";
      bleBtn.className = "btn secondary";
    } else {
      bleBtn.innerText = "Connect ESP32";
    }
  }
...
async function detectFrame() {
  ...
  if (isPointerFeatureActive()) {
    // ---- MODE 1: Pointer Reader & Scene Description ----
    if (video.currentTime !== lastVideoTime) {
      ...
      if (pointerResult) {
        ...
        if (now - handCooldown > 4000) {
          handCooldown = now;
          playChirp('hand');
          // Start heavy OCR/Scene processing
          processROI(ctx, canvas.width, canvas.height, pointerResult.roiRect);
        }
      }
    }
    // ensure vibro is stopped
